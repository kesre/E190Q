...
namespace DrRobot.JaguarControl
{
    public class Navigation
    {
        ...
        {
            mic1Background = new double[backgroundDataPoints];
            mic2Background = new double[backgroundDataPoints];
            mic3Background = new double[backgroundDataPoints];
            mic4Background = new double[backgroundDataPoints];

            mic1 = new double[maxMicDataPoints];
            ...
            {
                mic1Background[i] = jaguarControl.realJaguar.GetSensorHumanMotion1();
                mic2Background[i] = jaguarControl.realJaguar.GetSensorHumanAlarm1();
                mic3Background[i] = jaguarControl.realJaguar.GetSensorHumanMotion2();
                mic4Background[i] = jaguarControl.realJaguar.GetSensorHumanAlarm2();
                Thread.Sleep(5);
            }
            mic1BackAve = mic1Background.Sum() / backgroundDataPoints;
            mic2BackAve = mic2Background.Sum() / backgroundDataPoints;
            mic3BackAve = mic3Background.Sum() / backgroundDataPoints;
            mic4BackAve = mic4Background.Sum() / backgroundDataPoints;
        }
        ...
        public void runControlLoop()
        {
            ...
            while (runThread)
            {
                ...
                if (!jaguarControl.Simulating()&&!micInit)
                {
                    Thread.Sleep(30000);
        ...
        public void UpdateSensorMeasurements()
        {
            ...
            else
            {
                ...
                try
                {
                    ...
                    if (micIndex < maxMicDataPoints)
                    {
                        mic1[micIndex] = jaguarControl.realJaguar.GetSensorHumanMotion1() - mic1BackAve;
...
//        // Microphone values
//        private Queue<float>[] mic;
//        private double[] micAng = { 0, Math.PI / 2, Math.PI, -Math.PI / 2};
//        private float[] micOffset = { 3076, 3062, 3076, 3062 };
//        private double micThresh = 7650;
//        private int numMics = 4;
//        private bool heardSound = false;
//        private int numMicSamples;
//        private int numMaxMicSamples = 1024;
//        private double thThresh = Math.PI * 180 / 5;
...
//            mic = new Queue<float>[numMics];
//            for (int i = 0; i < numMics; i++)
//            {
//                mic[i] = new Queue<float>();
//            }
//            numMicSamples = 0;
//        }
...
//                    // Drive the robot to a desired Point (lab 3)
//                    if (heardSound && deltaTh < thThresh)
//                    {
//                        // Start going forward
//                        wallPosing = true;
//                        heardSound = false;
//                    }
//                    else if (wallPosing && deltaTh < thThresh)
//                    {
//                        // Go forward 
//                        //WallPositioning();
//                    }
...
//                    // Update Microphone Measurements
//                    while (numMicSamples < numMaxMicSamples)
//                    {
//                        ++numMicSamples;
//                        mic[1].Enqueue(jaguarControl.realJaguar.GetSensorHumanMotion1() - micOffset[1]);
//                        mic[0].Enqueue(jaguarControl.realJaguar.GetSensorHumanAlarm1() - micOffset[0]);
//                        mic[3].Enqueue(jaguarControl.realJaguar.GetSensorHumanMotion2() - micOffset[3]);
//                        mic[2].Enqueue(jaguarControl.realJaguar.GetSensorHumanAlarm2() - micOffset[2]);
//                    }
...
//        private void updateDesiredThFromSound()
//        {
//            double soundAng = checkForSound();
//            if (soundAng != -1)
//            {
//                deltaTh = soundAng;
//                desiredT = NormalizeAngle(t + deltaTh);
//                heardSound = true;
//            }
//        }
...
//            double omegaL = .5 * (desiredV - desiredW / robotRadius);
//            double omegaR = .5 * (desiredV + desiredW / robotRadius);

//            double dPhiL = omegaL * robotRadius * 2 / wheelRadius;
//            double dPhiR = omegaR * robotRadius * 2 / wheelRadius;
...
//        public double getAmplitude(Queue<float> micSamples)
//        {
//            // run fft 
//            float[] data = micSamples.ToArray();
//            Fourier.FFT(data, numMicSamples / 2, FourierDirection.Forward);
//            return data[1] + data[2] + data[3];
//        }

//        public double checkForSound()
//        {
//            double maxVal = 0;
//            int maxPos = 0;
//            if (numMicSamples < numMaxMicSamples)
//            {
//                return -1;
//            }

//            double[] micVals = new double[4];
//            for (int i = 0; i < numMics; i++)
//            {
//                micVals[i] = getAmplitude(mic[i]);
//                mic[i].Clear();
//            }
//            numMicSamples = 0;
...
//            for (int i = 0; i < numMics; i++)
//            {
//                //curSum = micVals[(i - 1 + numMics) % numMics] +
//                //    micVals[i] +
//                //    micVals[(i + 1) % numMics];
//                curSum = micVals[i];
//                if (curSum > maxVal)
...
//            double angle = micAng[maxPos];
//            int prev = (maxPos - 1 + numMics) % numMics;
//            int next = (maxPos + 1) % numMics;
//            angle -= Math.Abs(micAng[prev] - micAng[maxPos]) / 2 * (micVals[prev] / micVals[maxPos]);
//            angle += Math.Abs(micAng[next] - micAng[maxPos]) / 2 * (micVals[next] / micVals[maxPos]);
//            return maxPos;
//        }
